...
type OutboundTraffictLog struct {
	gorm.Model
	...
	RequestName        string     `json:"request_name" gorm:"column:request_name;not null:false;type:VARCHAR(100);"`
...
func MigrateOutboundTraffictLog(db *gorm.DB) {
	db.Exec("ALTER TABLE outbound_traffict_logs ALTER COLUMN response_status_code DROP NOT NULL;")
}
